...
const processPrice = (data) => {
  ...
  logger.debug('game.processPrice: ' + JSON.stringify(numbers))

  numbers.pop()
  prices_cached[parseInt(numbers.pop())] = {price: parseInt(numbers[3]), updated: Date.now()}
  master.forward(data)
...
const prices = (resource_codes) => {
  ...
  for (let i = 0; i < resource_codes.length; i ++) {
    let code = parseInt(resource_codes[i])

    if (typeof prices_cached[code] === 'undefined' || Date.now() - prices_cached[code].updated > 18000) {
