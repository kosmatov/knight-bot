...
const processExchange = (data) => {
  ...
  if (flush_exchange_slots) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (cmd && res_id) {
        if (flush_exchange_slots < 2) {
          let params = lines[i].match(/\d+/g).slice(0, 2)
          params.unshift(j)
          ex_res_stock.push(params)
        }
        send(cmd.pop())
      } else {
...
const processPrice = (data) => {
  ...
  if (exchange_res_id == code) {
    if (parseInt(numbers[0]) > 0) {
      sendCommand(commands.sell, [exchange_res_id, numbers[0], '1000'].join('_'))
    }
    exchange_res_id = null
  }
...
const moveResToExchange = () => {
  for (let i = 0; i < ex_res_stock.length; i++) {
    sendCommand(commands.sell, ex_res_stock[i].join('_'))
  }
}
...
const exchangeRemoveResource = (res_id) => {
  ...
  setTimeout(() => exchange_res_id = null, 600000)
  moveResFromExchange()
}

const exchangeAddResource = (res_id) => {
  exchange_res_id = res_id
  setTimeout(() => exchange_res_id = null, 600000)
  sendCommand("buy", res_id)
}
...
const start = (key) => {
  ...
  return main_loop = setInterval(() => {
    if (minToBattle() > 238) {
      restart()
      moveResFromExchange(2)
      setTimeout(moveResToExchange, 90000)
    }
