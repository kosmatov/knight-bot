var
  ...
  seafront = false,
  exchange_slots = 0,
  flush_exchange_slots = null,
  exchange_res_id = null,
  exchange_res_count = null,
  exchange_res_calculate_price = false,
  ignore_castle = false,
  send_queue = [],
  wait_answer = false,
  restarted = false,
...
const processUnknown = (data) => {
  if (data.content.includes(contents.bath)) sendBath();
  master.forward(data)
...
const processExchange = (data) => {
  ...
  var content = data.content.substr(idx - 3)

  logger.debug("game.processExchange: resource ID: " + exchange_res_id)

  if (exchange_res_id) {
    var found_content = []

    while (idx > 0) {
      idx = content.search(contents.resources[exchange_res_id])
      logger.debug("game.processExchange: resource ID: " + exchange_res_id + " Idx: " + idx)

      if (idx > 0) {
        found_content.push(content.substr(idx).split("\n").slice(0, 2).pop())
        logger.debug("game.processExchange: " + content)
        content = content.slice(idx + 1)
      }
    }

    exchange_res_id = null
    content = found_content.join("\n")
  }

  if (flush_exchange_slots) {
    if (flush_exchange_slots < 2) ex_res_stock = {};
    ...
    for (let i = 0; i < lines.length; i++) {
      let cmd = lines[i].match(/\/rm_[a-z\d]+/g)

      if (cmd) {
        if (res_id) {
          let nums = lines[i].match(/\d+/g)
          if (nums) ex_res_stock[res_id] = nums[0];
        }
        send(cmd.pop())
      } else if (flush_exchange_slots < 2) {
        for (let j in contents.resources) {
          if (contents.resources[j] == lines[i]) {
            res_id = j
            break
          }
        }
...
const processStock = (data) => {
  if (!data.content.includes(contents.stock)) return false;

  if (exchange_res_id) {
    var
      lines = data.content.split("\n"),
      count = null,
      res_name = contents.resources[exchange_res_id]

    for (let i = 0; i < lines.length; i++) {
      if (lines[i].startsWith(res_name)) {
        count = lines[i].match(/\d+/g)
        break
      }
    }

    if (count) {
      sendCommand(commands.sell, [exchange_res_id, count[0], 1000])
    }
  }
...
const processNoPrice = (data) => {
  if (!data.content.includes(contents.no_price)) return false;

  if (exchange_res_id) {
    if (exchange_res_count) {
      sendCommand(commands.sell, [exchange_res_id, exchange_res_count, 1000].join('_'))
    } else {
      sendCommand(commands.stock)
    }
  }
...
const processPrice = (data) => {
  ...
  logger.debug('game.processPrice: code ' + code + ' cnt ' + numbers[0] + ' price ' + numbers[3])
  logger.debug('game.processPrice: resource to process: ' + exchange_res_id)

  prices_cached[code] = {price: parseInt(numbers[3]), updated: Date.now()}

  if (exchange_res_id == code) {
    if (parseInt(numbers[0]) > 0) {
      let price = exchange_res_calculate_price ? prices_cached[code].price - 1 : 1000
      let count = exchange_res_count || numbers[0]
      sendCommand(commands.sell, [exchange_res_id, count, price].join('_'))
    }
    exchange_res_id = null
    exchange_res_count = null
  }

  if (data.chat_key != master.chat_key()) {
    let lines = data.content.split("\n")
    master.send(lines.slice(0, 2).join("\n") + ' ' + data.content.match(/\/wtb_\d+_1/g))
  }
...
const moveResToExchange = () => {
  ...
  for (let i in ex_res_stock) {
    setTimeout(() => exchangeAddResource(i, true, ex_res_stock[i]), j++ * 5000)
  }
}

const moveResFromExchange = (flag) => {
  flush_exchange_slots = flag || 1
  send(commands.exchange)
  setTimeout(() => flush_exchange_slots = null, 600000)
}

const exchangeRemoveResource = (res_id) => {
  exchange_res_id = res_id
  moveResFromExchange()
}

const exchangeAddResource = (res_id, calculate_price, count) => {
  exchange_res_id = res_id
  exchange_res_count = count
  exchange_res_calculate_price = calculate_price
  sendCommand(commands.buy, res_id)
}
...
const prices = (resource_codes) => {
  ...
  for (let i = 0; i < resource_codes.length; i ++) {
    ...
    if (typeof prices_cached[code] === 'undefined' || Date.now() - prices_cached[code].updated > 1800000) {
...
const start = (key) => {
  ...
  return main_loop = setInterval(() => {
    if (minToBattle() > 238) restartAfterBattle();
...
const restartAfterBattle = () => {
  if (restarted) return false;
  restart()
  setTimeout(() => moveResFromExchange(2), 300000)
  setTimeout(moveResToExchange, 390000)
}
...
module.exports = {
  ...
  exchangeAddResource: exchangeAddResource,
  exchangeRemoveResource: exchangeRemoveResource,
  ex_res_stock: () => ex_res_stock,
