...
const prices = () => {
  ...
  for (let key in prices) sorted_prices.push([key, prices[key]]);
  return sorted_prices.sort((a, b) => b[1] - a[1])
}
...
const buyResources = () => {
  ...
  for (let i = 0; i < prices.length; i ++) {
    if (hero.coins > prices[i][1]) {
      hero.coins -= prices[i][1]
      return buy(prices[i][0] + '_1')
    }
  }
}
