...
const canDoIt = {
  ...
  sleep: (chnc, hour) => options.sleep && (hour > 22 || hour < 2) && hero.stamina < 2 && game.minToBattle() > 60,
  buyResources: (chnc, hour) => hero.coins > options.max_coins && hero.level > 9 && banker.prices(),
  buyUrgent: (chnc, hour) => hero.coins > options.max_donate && game.minToBattle() < 40 && hero.level > 9 && banker.prices(),
  goCave: (chnc, hour) => last_action != 'goCave' && hero.stamina > 1 && chnc < options.cave_chance && hero.level > 6,
  goKorovan: (chnc, hour) => hero.stamina > 1 && (hour > 21 || hour < 4) && chnc > 100 - options.korovan_chance,
  goArena: (chnc, hour) => options.arena && last_action != 'cancelArena' && hero.coins > 4 && hour > 6 && hour < 21 && hero.level > 4 && arena_fights > 0 && game.minToBattle() > 20,
  goSeafront: (chnc, hour) => game.seafront() && chnc > 100 - options.seafront_chance && hero.stamina > 0 && hero.level > 14,
  goForest: (chnc, hour) => chnc < options.forest_chance && hero.stamina > 1,
  goRepair: (chnc, hour) => hero.level > 9 && chnc < 50 && !(hero.level > 14 && game.seafront()),
  goBuild: (chnc, hour) => hero.level > 9 && chnc > 49 && !(hero.level > 14 && game.seafront()),
}
...
const action = () => {
  ...
  if (isBusy()) {
    ...
    else if (arenaTimeout()) cancelArena();
    else return false;
  }

  if (game.minToBattle() < options.battle_threshold) {
    logger.debug('hero.action: goBattle')
    return goBattle()
  }
...
const prices = () => {
  var prices = banker.prices()
      res = []

  logger.debug('hero.buyResources: ' + JSON.stringify(prices))

  for (let i = 0; i < options.buy.length; i ++) {
    let key = parseInt(options.buy[i])
    res.push(key + ' ' + prices[key] + contents.coins)
  }
...
const buyResources = () => {
  ...
  for (let i = 0; i < options.buy.length; i ++) {
    let key = parseInt(options.buy[i])
    if (hero.coins > prices[key]) {
      hero.coins -= prices[key]
      return buy(key + '_1')
    }
  }
}
...
const setOption = (key, value) => {
  ...
  switch (typeof options[key]) {
    case 'object':
      options[key] = typeof value === 'object' ? value : [value]
      break
    case 'string':
      options[key] = value
      break
