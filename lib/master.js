...
const prices = () => {
  ...
  for (let i = 0; i < hprices.length; i ++) {
    res.push(hprices[i][0] + ' ' + settings.hero.contents.coins + hprices[i][1]);
  }
...
const processMessage = (data) => {
  ...
  if (cmd.startsWith('-') || cmd.match(/^s[\s\d]+/)) {
    return game.sendCommand('/wts_' + cmd.slice(1).trim().replace(/\s+/g, '_'))
  }
  ...
  if (data.type == 'message') game.processPrice(data);
